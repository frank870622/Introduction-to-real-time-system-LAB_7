...
int main()
{
    ...
    */
    ...
    while (1)
    {
        ...
        sort(readyvector_low.begin(), readyvector_low.end(), sort_slack_time);
        sort(waitvector.begin(), waitvector.end(), sort_realease_time);
        ...
        if (waitvector.size() > 0)
        {
            ...
            while (waitvector.begin()->release_time <= process_time && waitvector.size() > 0)
            {
                ...
                sort(readyvector_low.begin(), readyvector_low.end(), sort_slack_time); //sort the ready queue by execute time
        ...
        for (int i = 0; i < processor_num; i++)
        {
            ...
            if (runvector[i].size() <= 0)
            {
                ...
                else if (readyvector_low.size() > 0)
                {
                    ...
                    sort(readyvector_low.begin(), readyvector_low.end(), sort_slack_time); //sort ready queue with deadline
            ...
            //context switch
            if (runvector[i].size() > 0 && readyvector_low.size() > 0)
            {
        ...
        */
        sort(readyvector_low.begin(), readyvector_low.end(), sort_slack_time); //sort low ready queue by waiting time
        //add process time
        ++process_time;
    }
